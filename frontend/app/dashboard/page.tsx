  ...
  const handlePdfUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        setUploadStatus(error.response?.data?.detail || "PDF upload failed.");
      } else {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center justify-center rounded-md border border-zinc-300 bg-white px-4 py-1 text-base font-medium text-zinc-900 hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
          ...
          {uploadStatus && (
            <p className="text-sm text-zinc-500">{uploadStatus}</p>
          )}
        </form>
      </section>
    </main>
